  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm space-y-6">
      <div className="flex items-center space-x-3">
        ...
        <h2 className="text-xl font-semibold text-gray-900">Configuração do Lote {stats.currentBatch || 1}/{stats.totalBatches || 1}</h2>
      </div>

      {/* Alerta de Processamento em Lotes */}
      {stats.totalBatches && stats.totalBatches > 1 && (
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg p-6 border border-amber-200">
          <div className="flex items-start space-x-3">
            <AlertTriangle className="h-6 w-6 text-amber-600 mt-0.5" />
            <div>
              <h3 className="font-semibold text-amber-800 mb-2">⚠️ Processando Lote {stats.currentBatch}/{stats.totalBatches}</h3>
              <p className="text-amber-700">
                Você está configurando o lote <strong>{stats.currentBatch}</strong> de <strong>{stats.totalBatches}</strong>. 
                Este lote contém <strong>{stats.totalContacts} contatos</strong> e criará no máximo <strong>{Math.min(stats.estimatedGroups, 5)} grupos</strong>.
              </p>
              <div className="mt-3 p-3 bg-amber-100 rounded border border-amber-300">
                <p className="text-sm text-amber-800 font-medium">
                  🛡️ <strong>IMPORTANTE:</strong> Após processar este lote, aguarde pelo menos 24 horas antes de processar o próximo lote.
                  Isso é essencial para evitar banimento.
                </p>
              </div>
            </div>
          </div>
      ...
      {safeStats.totalContacts > 0 && (
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200">
          <h3 className="font-semibold text-green-900 flex items-center space-x-2 mb-6">
            ...
            <span>Estatísticas do Lote {stats.currentBatch || 1}/{stats.totalBatches || 1}</span>
          </h3>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
            <div className="bg-white rounded-lg p-4 text-center border border-green-200 shadow-sm">
              ...
              <div className="text-xs text-gray-600">Contatos (lote atual)</div>
            </div>
      ...
      <div className="bg-gradient-to-r from-red-50 to-pink-50 border border-red-200 rounded-lg p-6">
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h3 className="font-semibold text-red-800 mb-2">🛡️ PROCESSAMENTO EM LOTES SEGURO</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-red-700">
              <div>
                <p className="font-medium mb-1">Limites por Lote:</p>
                <ul className="space-y-1">
                  <li>• <strong>Máximo 250 contatos</strong> por lote</li>
                  <li>• <strong>Máximo 5 grupos</strong> por lote</li>
                  <li>• <strong>Máximo 50 contatos</strong> por grupo</li>
                  <li>• <strong>Máximo 3 grupos</strong> por sessão</li>
                </ul>
              </div>
              <div>
                <p className="font-medium mb-1">Cronograma Seguro:</p>
                <ul className="space-y-1">
                  <li>• <strong>Processe 1 lote por dia</strong></li>
                  <li>• <strong>Aguarde 24h entre lotes</strong></li>
                  <li>• <strong>Total: {stats.totalBatches || 1} dias</strong> para processar tudo</li>
                  <li>• <strong>Não tente acelerar</strong> o processo</li>
                </ul>
              </div>
            </div>
            <div className="mt-4 p-3 bg-red-100 rounded border border-red-300">
              <p className="text-xs text-red-800 font-medium">
                ⚠️ <strong>IMPORTANTE:</strong> Seus 4756 contatos serão processados em {stats.totalBatches || 20} lotes de 250 contatos.
                Processe um lote por dia e aguarde 24h entre lotes para evitar banimento.
              </p>
            </div>
          </div>
        </div>
