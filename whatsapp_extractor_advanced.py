#!/usr/bin/env python3
"""
WhatsApp Advanced Contact Extractor Tool
Ferramenta avançada para extrair contatos de grupos do WhatsApp Web
...
Uso:
1. Execute: python whatsapp_extractor_advanced.py
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('whatsapp_extraction.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class WhatsAppAdvancedExtractor:
    def __init__(self):
        self.page = None
        self.browser = None
        ...
        logger.info("🔍 Extrator avançado inicializado")
    
    async def start_browser(self):
        """Inicia o navegador e abre o WhatsApp Web"""
        try:
            logger.info("🌐 Iniciando navegador para extração...")
    ...
    async def get_all_groups(self):
        ...
        try:
            ...
            if not chat_list:
                raise Exception("Lista de chats não encontrada")
            
            # Busca elementos de grupo
            group_elements = await self.page.query_selector_all('[data-testid="cell-frame-container"]')
            groups = []
            
            for element in group_elements:
                try:
                    # Verifica se é um grupo
                    group_indicator = await element.query_selector('[data-testid="group-icon"]')
                    if group_indicator:
                        # Extrai nome do grupo
                        name_element = await element.query_selector('[data-testid="conversation-title"]')
                        if name_element:
    ...
    async def extract_contacts_from_group(self, group):
        ...
        try:
            ...
            logger.info(f"👥 Extraindo contatos de: {group_name}")
            ...
            if not info_clicked:
                logger.warning(f"⚠️  Não foi possível abrir info do grupo {group_name}")
                return []
            
            await asyncio.sleep(3)
            
            # Procura lista de participantes
            ...
            for element in participant_elements:
                try:
                    ...
                    # Extrai número
                    number_element = await element.query_selector('[data-testid="subtitle"]')
                    number = ''
                    if number_element:
                    ...
                    if self.config['include_admins']:
                        admin_element = await element.query_selector('[title*="admin"], [title*="Admin"]')
    ...
    async def extract_all_contacts(self):
        ...
        try:
            ...
            for i, group in enumerate(groups, 1):
                ...
                # Delay entre grupos
                if i < len(groups):
                    delay = (self.config['delay_min'] + self.config['delay_max']) / 2
    ...
    async def run(self):
        ...
        try:
            logger.info("🔍 Iniciando WhatsApp Advanced Extractor")
            logger.info("="*50)
            ...
            logger.info(f"📊 Total extraído: {len(self.extracted_contacts)}")
...
# Função principal
async def main():
    print("🔍 WhatsApp Advanced Contact Extractor")
    print("="*50)
    print("Ferramenta gerada automaticamente pela interface web")
    print("="*50)
    
    # Configurações opcionais
    extractor = WhatsAppAdvancedExtractor()
    
    group_filter = input("🔍 Filtrar grupos por nome (Enter para todos): ").strip()
    if group_filter:
