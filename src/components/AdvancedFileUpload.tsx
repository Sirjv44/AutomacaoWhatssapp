...
import { Upload, FileText, AlertCircle, CheckCircle, AlertTriangle, Users, Crown, Shield } from 'lucide-react';
...
export const AdvancedFileUpload: React.FC<AdvancedFileUploadProps> = ({ 
  ...
  const validateCSV = (text: string): Contact[] => {
    ...
    dataLines.forEach((line, index) => {
      ...
      // Limpa e valida formato do número (deve incluir DDI)
      const cleanNumber = numero.replace(/[\s\-\(\)]/g, '');
      if (!/^(55)?[\d]{10,11}$/.test(cleanNumber)) {
        errors.push(`Linha ${lineNumber}: Número inválido "${numero}" - use formato 5562999999999 (com DDI)`);
        return;
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        ...
        onDragOver={(e) => {
          e.preventDefault();
          if (lgpdConsent) setIsDragOver(true);
        }}
      ...
      >
        {isValidating ? (
          <div className="flex flex-col items-center space-y-3">
            ...
            <p className="text-xs text-gray-500">Processando milhares de contatos com validação LGPD...</p>
          </div>
        ) : (
          <>
            <div className="mb-4">
            ...
            <p className={`text-sm font-medium mb-6 ${lgpdConsent ? 'text-blue-600' : 'text-gray-400'}`}>
              ⚡ Suporte para milhares de contatos com validação DDI
            </p>
            <input
              type="file"
              accept=".csv"
      ...
      {validationResult && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h3 className="text-lg font-semibold text-gray-900">Resultado da Validação LGPD</h3>
          </div>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-100">
        <h3 className="font-semibold text-blue-900 mb-3 flex items-center space-x-2">
          ...
          <span>Formato do Arquivo CSV para Automação Avançada</span>
        </h3>
        <div className="space-y-4">
          <p className="text-sm text-blue-800">
            O arquivo deve conter as seguintes colunas com validação DDI obrigatória:
          </p>
          ...
          <div className="bg-gray-900 rounded-lg p-4">
            <p className="text-xs text-gray-400 mb-2">Exemplo com DDI brasileiro (55):</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-green-50 rounded-lg p-4 border border-green-200">
              ...
              <p className="text-xs text-green-700">
                Distribuídos em grupos de até 999 membros cada
              </p>
            </div>
            <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
              ...
              <p className="text-xs text-purple-700">
                Adicionados e promovidos em todos os grupos
              </p>
            </div>
          </div>
          <div className="text-xs text-blue-700 space-y-1">
            <p>• <strong>DDI obrigatório:</strong> Números devem incluir código do país (55 para Brasil)</p>
            <p>• <strong>Formato aceito:</strong> 5562999999999 ou 62999999999 (DDI será adicionado automaticamente)</p>
            <p>• <strong>Validação LGPD:</strong> Consentimento obrigatório para processamento de dados</p>
            <p>• <strong>Grupos automáticos:</strong> Criação sequencial com até 999 membros</p>
